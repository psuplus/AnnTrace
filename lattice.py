...
class Lattice:
    ...
    def __init__(self):
        ...
        self.labels = [self.top, self.top]

    def add_sub(self, low:Label, high:Label) -> None:
        self.sub.append((low, high))
        if (low not in self.labels) and (low != self.bot):
            self.labels.append(low)
        if (high not in self.labels) and (high != self.top):
            self.labels.append(high) 
        if self.lowest == self.bot or self.check_sub(low, self.lowest):
            self.lowest = low

    def check_sub(self, low:Label, high:Label) -> bool:
    ...
    def join(self, l1:Label, l2:Label) -> Label:
...
if __name__ == '__main__': # Should pass all tests below
    l1 = Label("L1")
    l2 = Label("L2")
    l3 = Label("L3")
    l4 = Label("L4")
    ...
    test_lattice_sub(lattice, Label("L1"), l2, False)
    test_lattice_sub(lattice, Label("L1"), l3, True)
    ...
    test_lattice_sub(lattice, l4, Label("L6"), True)
    ...
    test_lattice_join(lattice, Label("L1"), l2, l3)
    ...
    test_lattice_join(lattice, l3, Label("L4"), lt)
    test_lattice_join(lattice, l3, l5, lt)
    test_lattice_join(lattice, l3, l6, lt)
    test_lattice_join(lattice, Label("L4"), l6, l6)
    ...
    test_lattice_join(lattice, Label("L4"), lb, l4)
